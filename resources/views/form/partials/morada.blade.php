    ...
    <div class="flex-1 w-full mx-2 ">
        <div class="flex p-1 my-2 bg-white border border-gray-200 rounded ">
            <textarea placeholder="morada completa" name="morada" rows="2" data-formulario="morada"
                ...
                class="w-full p-1 px-2 text-gray-800 rounded border-none appearance-none">{{ old('morada') }}</textarea>
        </div>
        ...
        <div class="flex-1 w-full mx-2 ">
            ...
            <div class="flex p-1 my-2 bg-white border border-gray-200 rounded ">
                <select name="distrito_id" data-formulario="distrito" onblur="validateField('distrito')" required
                    ...
                    <option value="" disabled {!! old('distrito_id')==null?"selected":"" !!}>distrito</option>
                    @foreach ($distritos as $distrito)
                        <option value="{{ $distrito->id }}" data-formulario-option="distrito"  {!! $distrito->id==old('distrito_id')?"selected":"" !!}>{{ $distrito->nome }}
                        </option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="flex-1 w-full mx-2 ">
            ...
            <div class="flex p-1 my-2 bg-white border border-gray-200 rounded ">
                <select name="concelho_id" data-formulario="concelho" onblur="validateField('concelho')" required
                    class="w-full p-1 px-2 text-gray-800 appearance-none rounded border-none" >
                    <option value="" {!! old('concelho_id')==null?"selected":"" !!} disabled>concelho</option>
                    @foreach ($concelhos as $concelho)
                        <option   {!! $concelho->id==old('concelho_id')?"selected":"" !!} {!! old('distrito_id')==$concelho->distrito_id?"":"class='hidden'" !!} value="{{ $concelho->id }}" data-formulario-option="concelho"
        ...
        <div class="mx-2 w-full md:w-2/3 lg:w-2/3 xl:w-2/3">
            ...
            <div class="flex p-1 my-2 bg-white border border-gray-200 rounded ">
                <select name="estado_civil_id" data-formulario="estadoCivil" 
                    class="w-full p-1 px-2 text-gray-800 appearance-none rounded border-none">
                    <option value="" {!! old('estado_civil_id')==null?"selected":"" !!} disabled>indique o seu estado civil</option>
                    @foreach ($estadosCivis as $estado)
                        <option value="{{ $estado->id }}" {!! old('estado_civil_id')== $estado->id?"selected":"" !!}>{{ $estado->tipo }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="mx-2 w-full md:w-1/3 lg:w-1/3 xl:w-1/3">
            ...
            <div class="flex p-1 my-2 bg-white border border-gray-200 rounded ">
                <input placeholder="8500" name="postal1" id="postal1" data-target="postal1" data-formulario="postal1" 
                    pattern="[0-9]{4}" maxlength="4" onblur="validateField('postal1')" required value="{{ old('postal1') }}"
                ...
                <input placeholder="000" name="postal2" id="postal2" data-target="postal2" data-formulario="postal2"
                    pattern="[0-9]{3}" required maxlength="3" onblur="validateField('postal2')" required value="{{ old('postal2') }}"
